...
export default function CommunicationDetail({
  ...
  const principalAuthor = communication.authors.find(
    (author: any) => author.role === "principal"
  );
  const coauthors = communication.authors.filter(
    (author: any) => author.role === "coautor"
  );
  if (!communication) {
    return null;
  }
            ...
            <div className='flex-1'>
              ...
              <div className='flex flex-wrap items-center gap-3 text-sm text-muted-foreground mb-4'>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  {communication.authors.length} autor
                  {communication.authors.length > 1 ? "es" : ""}
                </div>
              </div>
            ...
            <div className='flex flex-col items-end justify-between'>
              <div className='text-right mb-2'>
                ...
                <div className='font-medium'>
                  {communication.congress?.name || communication.congressId}
                </div>
              </div>
      ...
      <Tabs defaultValue='details' className='w-full'>
        ...
        <TabsContent value='details' className='space-y-4 pt-4'>
          <Card>
            ...
            <CardContent className='space-y-4'>
              ...
              {communication.status === "accepted" &&
                communication.presentationDetails && (
                  <div className='p-4 bg-green-50 border border-green-200 rounded-lg'>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                      <div className='flex items-center gap-2 text-green-700'>
                        <Calendar className='w-4 h-4' />
                        <span>
                          Fecha: {communication.presentationDetails.date}
                        </span>
                      </div>
                      <div className='flex items-center gap-2 text-green-700'>
                        <Clock className='w-4 h-4' />
                        <span>
                          Hora: {communication.presentationDetails.time}
                        </span>
                      </div>
                      <div className='flex items-center gap-2 text-green-700'>
                        <MapPin className='w-4 h-4' />
                        <span>
                          Sala: {communication.presentationDetails.room}
                        </span>
                      </div>
                      <div className='flex items-center gap-2 text-green-700'>
                        ...
                        <span>
                          Moderador:{" "}
                          {communication.presentationDetails.chairperson}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {communication.reviewComments &&
            communication.reviewComments.length > 0 && (
              <Card>
                <CardHeader>
            ...
            <CardHeader>
              <CardTitle className='flex items-center justify-between'>
                ...
                <Badge variant='secondary'>
                  {communication.authors.length} autor(es)
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className='space-y-4'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='text-right'>
                      <div className='text-sm font-semibold text-green-600'>
                        {communication.paymentStatus === "paid"
                          ? "Pagado"
                          : "€8.00"}
                      </div>
        ...
        <TabsContent value='files' className='space-y-4 pt-4'>
          <Card>
            ...
            <CardContent className='space-y-4'>
              {Object.entries(communication.files).map(
                ([key, file]: [string, any]) => {
                  let icon;
                  let bgColor;
                  let textColor;
                  ...
                  return (
                    <div
                      key={key}
                      className={`p-4 ${bgColor} rounded-lg border flex items-center gap-4`}>
                      <div
                        className={`p-3 bg-white/50 rounded-full ${textColor}`}>
                        {icon}
                      </div>
                      <div className='flex-1'>
                        <h4 className={`font-medium ${textColor} capitalize`}>
                          {key}
                        </h4>
